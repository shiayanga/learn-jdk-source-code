...
package com.gitee.shiayanga.hashmap;
...
public class HashMap<K, V> extends AbstractMap<K, V>
    ...
    /**
     * 最大容量，如果构造函数参数指定的值大于 MAXIMUM_CAPACITY 的话，就使用 MAXIMUM_CAPACITY
     * MAXIMUM_CAPACITY 的值必须是2的n次方，且小于等于2的30次方
     */
    static final int MAXIMUM_CAPACITY = 1 << 30;

    /**
     * 构造函数中未指定负载因子时，默认使用的负载因子是 0.75
     */
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        Node<K, V> p;
        int n, i;
        // 如果当前 HashMap 是空的，也就是说刚刚初始化的，此时 resize 之后 n 的值是16
        if ((tab = table) == null || (n = tab.length) == 0) {
            n = (tab = resize()).length;
        }

        // 如果需要放置键值的位置是空的，就将键值放进去
    ...
    final Node<K, V>[] resize() {
        ...
        // 如果容量不为空
        if (oldCap > 0) {
            // 如果原先的 HashMap 容量已经是最大，就将原先的 HashMap 返回
            if (oldCap >= MAXIMUM_CAPACITY) {
                threshold = Integer.MAX_VALUE;
                return oldTab;
            }
            // 如果原先的HashMap容量*2小于最大容量，同时原先的HashMap容量大于等于默认的初始化容量，那么新的阈值就是原先的容量*2
            else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&
                    oldCap >= DEFAULT_INITIAL_CAPACITY) {
                newThr = oldThr << 1; // double threshold
            }
        } else if (oldThr > 0) {
            // 如果此时的HashMap没有存储过数据，那么阈值就是默认的初始化大小，所以此时HashMap的容量就是初始大小16
            // 这时阈值不为空的原因是，在初始化HashMap的时候，使用的不是无参构造方法
            newCap = oldThr;
        } else {
